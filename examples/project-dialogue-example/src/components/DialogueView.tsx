      ...
      <div className="max-w-2xl w-full">
        {currentMessage ? (
          <div className={`transform transition-all duration-300 ${
            currentMessage.speaker === 'system' ? 'slide-in-left' : 'slide-in-right'
          }`}>
            <div className={`flex items-start gap-4 ${
              currentMessage.speaker === 'user' ? 'flex-row-reverse' : ''
            }`}>
              <div className={`w-16 h-16 rounded-full flex items-center justify-center text-white text-2xl font-bold ${
                currentMessage.speaker === 'system' ? 'bg-green-500' : 'bg-blue-500'
              }`}>
              ...
              <div className={`flex-1 p-6 rounded-lg ${
                ...
                <p className="text-lg" dangerouslySetInnerHTML={{ __html: currentMessage.text.replace(/\n/g, '<br />') }} />
                {currentMessage.speaker === 'system' && currentStepType === 'notice' && onNextClick && (
